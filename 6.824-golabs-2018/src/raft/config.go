...
func (cfg *config) start1(i int) {
	...
	fmt.Println("### start->goroutine110")
	applyCh := make(chan ApplyMsg)
	fmt.Println("### start->goroutine111")
	go func() {
		fmt.Println("### start->goroutine")
		for m := range applyCh {
			fmt.Printf("### start->goroutine, get messages from applyCh %v\n", m)
			err_msg := ""
...
func (cfg *config) nCommitted(index int) (int, interface{}) {
	...
	for i := 0; i < len(cfg.rafts); i++ {
		...
		//fmt.Printf("***### in nCommitted(, server %d could get index %d total count is %d\n", i, index, count)
	}
	return count, cmd
}
...
func (cfg *config) one(cmd int, expectedServers int, retry bool) int {
	fmt.Printf("***@@@ calling test func one, with (%d, %d)\n", cmd, expectedServers)
	t0 := time.Now()
	starts := 0
